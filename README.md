...
节点导入后，还需要绑定 CF 优选域名才能在订阅中生成 CDN 代理。

**导入第三方优选域名：**

1. PigeonRelay → **本地优选** → 添加域名映射
2. 填写第三方优选域名的 FQDN（如 `cf.090227.xyz`），来源选 `external`
3. 重复添加所有需要的第三方域名

**绑定到节点：**

1. 节点管理 → hover 节点 → **设置** → CF 优选绑定 → 添加
2. 来源选 `外部` → 选择刚导入的域名 → 填写名称前缀（如 `三方优选1`）
3. 或使用**批量添加**一次性粘贴多个域名

> 每个优选域名绑定会在订阅中生成一条独立的代理配置。如需创建自己的优选域名并自动更新 DNS，详见 [docs/local-opt-guide.md](docs/local-opt-guide.md)
