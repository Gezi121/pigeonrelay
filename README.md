...
节点导入后，还需要绑定 Cloudflare 优选域名才能在订阅中生成 CDN 代理：

1. **导入优选域名**：PigeonRelay → **本地优选** → 添加域名映射（或批量创建）。填入子域名和完整域名，选择算法类型
2. 在 Cloudflare DNS 中为这些子域名创建 **灰云 A 记录**（内容随便填，后续系统自动更新）
3. **绑定到节点**：节点管理 → hover 节点 → **设置** → CF 优选绑定 → 添加
4. 选择来源（本地/外部）→ 选择刚创建的域名 → 填写名称前缀 → 保存

> 每个优选域名绑定会在订阅中生成一条独立的代理配置。详见 [docs/local-opt-guide.md](docs/local-opt-guide.md)

### 第六步：创建订阅

1. **Sub Links** → 新建 → 填写名称
2. 可选：设置密码、流量上限 (GB)、到期日期
3. 页面提供 **Clash** 和 **V2Ray** 两个导出按钮，分别复制对应格式
4. 粘贴到客户端：Mihomo / Clash Verge / Shadowrocket / v2rayN
...
以下功能后端已预留接口，前端和完整流程尚未就绪：

- **多用户与配额** — 用户创建、流量配额、到期控制
- **配置备份与恢复** — 创建配置快照、一键恢复
- **Webhook 通知** — 到期提醒、流量告警
